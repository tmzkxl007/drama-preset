...
벤치마킹 대상: 유튜브 @포레이로. 2026-09-05 에 그 채널 10편을 전부 받아 재서 만든 값이다.
근거·실측 과정은 `docs/PLAYBOOK.md` §17, 원자료는 `forey_probe/`(analysis2.json 등).

★딸기우유와 완전히 다른 규격이다. 바탕값(_engine/base.py)에서 화면·색·자리·길이를 전부 갈아엎는다.
  가장 큰 차이는 **나레이션이 주인이고 원본 대사는 양념**이라는 것이다(§17-6).
"""
...
# ── 화면 (10편 전부 동일. 흔들림 없음) ──────────────────
CANVAS = (1080, 1920)
PIC = (1080, 1014)          # 소재가 놓이는 칸 — 16:9 원본의 좌우를 잘라 1.065:1 로
VID_Y = 432                 # 소재 위쪽 y  (윗 검은 띠 = 0~431, 432px)
BOTTOM_BAND = 474           # 아래 검은 띠 1446~1919
...
HEAD_Y = (176, 282)         # 두 행의 잉크 위쪽 y — 줄간격 32→16 (사용자 2026-09-15 "줄간격 줄이고 폰트 키워")
HEAD_INK = (90, 90)         # ★목표 잉크 높이 80→90
...
COL_HEAD2 = "&H001200E6"    # 빨강 #E60012 (예전 흰색)
...
NARR_CAP_Y = 1075           # ★사용자 지시로 나레를 위로 올린다 (원본은 대사와 같은 자리)
DLG_CAP_Y = 1272            # 대사는 아래 — 나레와 겹치지 않게 벌린다
...
#   ★2026-09-19 사용자 확정(mk13c 샘플 "이 비율로 확정"): 나레는 장면 잇는 한 줄씩 5~6마디(≤18자, 60~90자),
#     시간 비중 20~25%. 대사가 본문 — 10블록 안팎, 시간 70% 이상. 나레 두 마디 연속 금지(build.py 검사).
TOME = dict(NARR_SHARE=0.15, NARR_CHARS=(60, 100), NARR_BEATS=(5, 7),
            LEN_RANGE=(35, 60), DLG_MARKS=(8, 14), CUTS_PER_MIN=(25, 55),
            END_OPEN=False)  # END_OPEN=False = 마지막을 맺어도 된다
DLG_MARKS = (2, 6)          # 원본 대사가 끼어드는 횟수
NARR_CPS = (4.0, 5.5)       # 나레 초당 글자수
NARR_BEATS = (8, 15)        # 나레 마디 개수. ★사용자 지시로 하한을 8 로 낮췄다(나레를 줄인다)
NARR_BEAT_SECS = (1.2, 3.2)  # 한 마디 길이. ★2026-09-19 마디 ≤18자·1.3배라 1.2~3.2초
NARR_SECS = NARR_BEAT_SECS   # 바탕값 이름도 맞춰 둔다

# ── 컷 ──────────────────────────────────────────────────
CUTS_PER_MIN = (38, 47)     # 실측 38.3~47.3
CUT_AVG = (1.23, 1.49)
CUT_OPEN = 5                # ★첫 5초 안에 4~6컷. 앞만 빠르게 몰아친다
